...
/**
 * @ngdoc function
* @name staticApp.controller:AllFundingCtrl
 * @description
 * # AllFundingCtrl
...
angular.module('staticApp')
  .controller('AllFundingCtrl', function ($scope, $stateParams, $state, $log, $modal, OrganizationService, TableService, LookupService, NotificationService, StateService, BrowserService, smoothScroll) {
      ...
      $scope.onEditIconClick = function (org) {
          $state.go('funding.edit', { organizationId: org.organizationId });
      }

      $scope.addOrganization = function () {
          var addOrganizationModalInstance = $modal.open({
              animation: true,
              templateUrl: 'app/funding/add-funding-modal.html',
              controller: 'AddFundingModalCtrl',
              backdrop: 'static',
              size: 'lg'
          });
          addOrganizationModalInstance.result.then(function (addedOrganization) {
              $log.info('Finished adding funding source.');
              addOrganizationModalInstance.close(addedOrganization);
              StateService.goToFundingState(addedOrganization.organizationId);
          }, function () {
