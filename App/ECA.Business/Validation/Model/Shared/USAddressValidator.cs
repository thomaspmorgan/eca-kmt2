...
namespace ECA.Business.Validation.Model.Shared
{
    public class USAddressValidator : AbstractValidator<USAddress>
    {
        ...
        {
            RuleFor(visitor => visitor.Address1).NotNull().WithMessage("Address: Address Line 1 is required").Length(1, ADDRESS_MAX_LENGTH).WithMessage("Address: Address Line 1 can be up to " + ADDRESS_MAX_LENGTH.ToString() + " characters");
            ...
            RuleFor(visitor => visitor.PostalCode).NotNull().WithMessage("Address: Postal Code is required").Length(POSTAL_CODE_LENGTH).WithMessage("Address: Postal Code must be " + POSTAL_CODE_LENGTH.ToString() + " digits").Matches(@" ^\d{5}$").WithMessage("Address: Postal Code must be numeric");
