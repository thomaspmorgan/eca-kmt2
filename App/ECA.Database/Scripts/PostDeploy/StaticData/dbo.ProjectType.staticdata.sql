...
-- your Team Database project for [dbo].[ProjectType].

PRINT 'Updating static data table [dbo].[ProjectType]'
...
INSERT INTO [dbo].[ProjectType] ([ProjectTypeId], [ProjectTypeName], [History_CreatedBy], [History_CreatedOn], [History_RevisedBy], [History_RevisedOn])
SELECT tmp.[ProjectTypeId], tmp.[ProjectTypeName], tmp.[History_CreatedBy], tmp.[History_CreatedOn], tmp.[History_RevisedBy], tmp.[History_RevisedOn]
FROM @tblTempTable tmp
LEFT JOIN [dbo].[ProjectType] tbl ON tbl.[ProjectTypeId] = tmp.[ProjectTypeId]
WHERE tbl.[ProjectTypeId] IS NULL
SET IDENTITY_INSERT [dbo].[ProjectType] OFF

-- 4: Update any modified values with the values from the table variable
UPDATE LiveTable SET
LiveTable.[ProjectTypeName] = tmp.[ProjectTypeName],
...
FROM [dbo].[ProjectType] LiveTable 
INNER JOIN @tblTempTable tmp ON LiveTable.[ProjectTypeId] = tmp.[ProjectTypeId]

-- 5: Delete any missing records from the target
IF @DeleteMissingRecords = 1
BEGIN
	DELETE FROM [dbo].[ProjectType] FROM [dbo].[ProjectType] LiveTable
	LEFT JOIN @tblTempTable tmp ON LiveTable.[ProjectTypeId] = tmp.[ProjectTypeId]
	WHERE tmp.[ProjectTypeId] IS NULL
END

PRINT 'Finished updating static data table [dbo].[ProjectType]'
