...
/* Change Region for U.S. From US to WHA */
select * from location where locationtypeid = 3 and locationname = 'United States'
select * from location where locationtypeid = 2
--update location set region_locationid = 6,locationtypeid = 3 where locationid = 193
select * from location where region_locationid = 1592


/* Update the CountryId where DivisionId Exists */
UPDATE dbo.location 
SET country_locationid = 
(SELECT l.country_locationid 
FROM dbo.location l 
WHERE l.locationid = location.division_locationid)
WHERE division_locationid is not null
