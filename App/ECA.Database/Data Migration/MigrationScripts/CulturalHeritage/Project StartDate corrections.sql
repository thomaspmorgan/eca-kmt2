...
FETCH NEXT FROM @cursorProject INTO @AFCPId, @title, @summary, @projectid, @expiresdate , @startdate

/* Loop thru all projects (staging) - should match DB */
WHILE @@FETCH_STATUS = 0
BEGIN

  /* Get the projectid for the title/summary to eliminate duplicates */
--  SELECT @projectid = projectid 
--    FROM eca_dev.dbo.program 
--   WHERE name = @title AND description = @summary
