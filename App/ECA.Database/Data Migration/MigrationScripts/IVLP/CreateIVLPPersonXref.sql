...
LEFT JOIN ECA_Dev.eca_dev.dbo.location city ON (city.locationtypeid = 5 AND 
        /* Birth City Null and birth country Exists */
     	((city.locationname IS NULL AND p.birth_city IS NULL AND city.country_locationid = birthcountry.locationid) OR
        /* Birth City Exists and Birth Country does not exist OR Birth Country DOES exist */      
	(city.locationname = p.BIRTH_CITY AND ((city.country_locationid IS NULL AND birthcountry.locationid IS NULL) OR city.country_locationid = birthcountry.locationid))))
...
/* This is corrected code that works following removal of not null constraint on Person.DateofBirth column */
    ...
    ((m.maritalstatusid is null and ep.maritalstatusid is null) OR (m.MaritalStatusId = ep.MaritalStatusId)) AND

    ((cp.birth_city IS NULL AND cp.birth_country IS NULL AND ep.PlaceOfBirth_LocationId IS NULL) OR (ep.PlaceOfBirth_LocationId = city.locationid))
