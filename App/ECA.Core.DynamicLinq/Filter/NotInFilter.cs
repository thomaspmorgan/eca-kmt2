...
{
    /// <summary>
    /// An NotInFilter is used to filter objects whose property value is either null or not contained in a collection of items.
    /// </summary>
    /// <typeparam name="T">The type of object to filter.</typeparam>
    public class NotInFilter<T> : BinaryFilter<T> where T : class
    {
        /// <summary>
        ...
        public NotInFilter(string property, object value)
            : base(property, value)
        {
        ...
        /// <summary>
        /// Returns a where expression checking the property value will be null or not contained in an IEnumerable of items.
        /// </summary>
        /// <returns>The where expression checking an enumerable object not containing the property to filter with or the property is null.</returns>
        public override Expression<Func<T, bool>> ToWhereExpression()
        {
            ...
            if (IsNullable)
            {
                ...
                where = Expression.AndAlso(hasValueExpression, Expression.Not(callContainsMethod));
                where = Expression.Or(Expression.IsFalse(hasValueExpression), where);
            }
            else
            {
                where = Expression.Not(Expression.Call(containsMethod, list, xProperty));
            }
