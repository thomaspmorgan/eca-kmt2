...
namespace ECA.Business.Test.Service.Reports
{
    ...
    public class ReportServiceTest
    {
        ...
        public void TestGetPostAwards()
        {
            ...
            var postInFrance = new Location
            {
                LocationId = post1,
                LocationName = "Paris",
                LocationTypeId = LocationType.Post.Id,
                CountryId = countryId1,
                RegionId = regionIdEUR
            };

            var postInSouthAfrica = new Location
            {
                LocationId = post2,
            ...
            var regionAward1 = new PostAwardDTO
            {
                Post = "Paris",
                Region = "EUR",
                ProgramValue = value1 + value2,
            ...
            var regionAward2 = new PostAwardDTO
            {
                Post = "Pretoria",
                Region = "AF",
                ProgramValue = value4,
            ...
            var postAwards = new List<PostAwardDTO>();

            postAwards.Add(regionAward1);
            postAwards.Add(regionAward2);
            ...
            locationInSouthAfrica.Region = regionAF;
            locationInFrance.Region = regionEUR;
            postInFrance.Region = regionEUR;
            postInSouthAfrica.Region = regionAF;

            context.Locations.Add(regionEUR);
            context.Locations.Add(regionAF);
            context.Locations.Add(locationInFrance);
            context.Locations.Add(locationInSouthAfrica);
            context.Locations.Add(postInFrance);
            context.Locations.Add(postInSouthAfrica);
            ...
            Action<IQueryable<PostAwardDTO>> tester = (resultAwards) =>
            {
                var resultList = resultAwards.ToList<PostAwardDTO>();
                CollectionAssert.AreEqual(resultList.Select(p => p.Post).ToList(), postAwards.Select(p => p.Post).ToList());
                CollectionAssert.AreEqual(resultList.Select(p => p.Region).ToList(), postAwards.Select(p => p.Region).ToList());
                CollectionAssert.AreEqual(resultList.Select(p => p.ProgramValue).ToList(), postAwards.Select(p => p.ProgramValue).ToList());
                CollectionAssert.AreEqual(resultList.Select(p => p.OtherValue).ToList(), postAwards.Select(p => p.OtherValue).ToList());
                CollectionAssert.AreEqual(resultList.Select(p => p.Projects).ToList(), postAwards.Select(p => p.Projects).ToList());
                CollectionAssert.AreEqual(resultList.Select(p => p.Average).ToList(), postAwards.Select(p => p.Average).ToList());
            };

            var result = service.GetPostAwards(programId);
            tester(result);
        }
